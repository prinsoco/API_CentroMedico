...
namespace ProcesoMedico.Api.Controllers.v1
{
    ...
    public class HorarioController : ControllerBase
    {
        ...
        [HttpPost("crear")]
        public async Task<IActionResult> Create([FromBody] Horario dto)
        {
            var id = await _service.InsertHorarioAsync(dto);
            var response = new ResponseCreate()
            {
                Id = id,
                Message = id > 0 ? "Horario registrado con exito" : "Error al registro el horario"
            };
        ...
        [HttpPut("update")]
        public async Task<IActionResult> Update([FromBody] Horario dto)
        {
            var affected = await _service.UpdateAsync(dto);
            var response = new ResponseCreate()
            {
                Id = affected,
                Message = affected > 0 ? "Horario editado con exito" : "Error al editar el Horario"
            };
            return Ok(response);
        }
        ...
        public async Task<IActionResult> GetAll([FromHeader] string? input, [FromHeader] string? combo)
        {
            ...
            return Ok(new ResponseDetails<List<Horario>>(items?.ToList()));
        }
