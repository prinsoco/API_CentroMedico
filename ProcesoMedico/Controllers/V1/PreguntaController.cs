...
namespace ProcesoMedico.Api.Controllers.v1
{
    ...
    public class PreguntaController : ControllerBase
    {
        ...
        public PreguntaController(IPreguntaService service) => _service = service;

        [HttpPost]
        public async Task<IActionResult> Create([FromBody] Pregunta dto)
        {
            var id = await _service.InsertPreguntaAsync(dto);
            return id > 0 ? Ok(new { Code = "00", Mensaje = "Pregunta registrada con exito" }) : Ok(new { Code = "99", Mensaje = "Error al registro la pregunta" });
        }

        [HttpPut("{id:int}")]
        public async Task<IActionResult> Update(int id, [FromBody] Pregunta dto)
        {
            var affected = await _service.UpdatePreguntaAsync(dto);
