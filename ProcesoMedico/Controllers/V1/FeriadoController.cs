...
namespace ProcesoMedico.Api.Controllers.v1
{
    ...
    public class FeriadoController : ControllerBase
    {
        ...
        public FeriadoController(IFeriadoService service) => _service = service;

        [HttpPost("crear")]
        public async Task<IActionResult> Create([FromBody] Feriado dto)
        {
            var id = await _service.InsertFeriadoAsync(dto);
            var response = new ResponseCreate()
            {
                Id = id,
                Message = id > 0 ? "Feriado registrado con exito" : "Error al registro el feriado"
            };
        ...
        [HttpPut("update")]
        public async Task<IActionResult> Update([FromBody] Feriado dto)
        {
            var affected = await _service.UpdateFeriadoAsync(dto);
            var response = new ResponseCreate()
            {
                Id = affected,
                Message = affected > 0 ? "Feriado editado con exito" : "Error al editar el feriado"
            };
            return Ok(response);
        }
        ...
        [HttpGet("getAll")]
        public async Task<IActionResult> GetAll([FromHeader] string? anio)
        {
            var items = await _service.ListAsync(new { Anios = anio });
            return Ok(new ResponseDetails<List<Feriado>>(items?.ToList()));
        }
