...
namespace ProcesoMedico.Api.Controllers.v1
{
    ...
    public class EncuestaController : ControllerBase
    {
        ...
        public EncuestaController(IEncuestaService service) => _service = service;

        [HttpPost]
        public async Task<IActionResult> Create([FromBody] Encuesta dto)
        {
            var id = await _service.InsertEncuestaAsync(dto);
            return id > 0 ? Ok(new { Code = "00", Mensaje = "Encuesta registrada con exito" }) : Ok(new { Code = "99", Mensaje = "Error al registro la encuesta" });
        }

        [HttpPut("{id:int}")]
        public async Task<IActionResult> Update(int id, [FromBody] Encuesta dto)
        {
            var affected = await _service.UpdateEncuestaAsync(dto);
